...
int main(int argc, char const* argv[]) {
	try {
		...
		const std::string emailFrom("uj07077@sju.ac.kr");
		const std::string emailTo("kjunwoo23@sju.ac.kr");
		const std::string message("Hello There~ I am THE MAWILE ~!");
		//const std::string serverPort(25);
		const std::string smtpServerStr("smtp.sejong.ac.kr");
		const std::string subject("Mawi1e's Test!");
